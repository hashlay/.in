...
exports.getStats = async (req, res) => {
  ...
  const data = await getOrSet('dashboard:stats', 45, async () => {
    ...
    const [
      ...
      Order.countDocuments({ isActive: true, orderStatus: 'delivered' }),
      Order.countDocuments({ isActive: true, orderStatus: 'cancelled' }),
      Order.aggregate([
        { $match: { isActive: true, createdAt: { $gte: today, $lt: tomorrow }, orderStatus: { $ne: 'cancelled' } } },
